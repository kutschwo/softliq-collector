...
int getParameter(cJSON* json, CONFIG* cfg)
{
    ...
    // Serial port
    value = cJSON_GetObjectItem(json, "softlic_url");
    if (value == NULL || !cJSON_IsString(value))
    {
        printf("Invalid value for softlic_url\n");
        return 10;
    }

    cfg->softlic_url = strdup(value->valuestring);

    // reset vbus (via serial device)
    value = cJSON_GetObjectItem(json, "post_string");
    if (value != NULL && value->valuestring != NULL) {
        ...
        cfg->post_string = strdup(value->valuestring);
    }
    ...
    cfg->print_stdout = value->valueint != 0;
